...
$(document).ready(function () {
    ...
    */
    function fetchData() {
        var html;
        fetch('https://f.hubspotusercontent20.net/hubfs/486579/LegalParse/EN/v4/EN_User_Products_Policy.html?update')
    ...
    */
    ...
    /*
    @Description: Replacing special characters and spaces in headings to be used as anchor
    */
    function cleanAnchors(heading) {
        return heading.trim().toLowerCase().replace(/ /g, "_").replace(/[|&;$%@"'â€™<>()+,]/g, "");
    }
    ...
    function innerHeaders() {
        ...
        $.each(headings, function () {
            if (headings[x].length > 1) {
                h_aux = '<div class="row"><div class="span12 sm_section"><span class="h2_title"><a href="#' + cleanAnchors(headings[x][0], x) + x + '" class="h2_title">' + headings[x][0] + '</a><img src="https://cdn2.hubspot.net/hubfs/486579/LegalParse/i/arrow_down.svg" class="nav_arrow_h2 collapsed"></span>';
                // h_aux = '<div class="row"><div class="span12 sm_section"><span class="h2_title"><a href="#title_h1_' + x + '" class="h2_title">' + headings[x][0] + '</a><img src="https://cdn2.hubspot.net/hubfs/486579/LegalParse/i/arrow_down.svg" class="nav_arrow_h2 collapsed"></span>';
            } else {
                h_aux = '<div class="row"><div class="span12 sm_section"><a href="#' + cleanAnchors(headings[x][0], x) + x + '" class="h2_title">' + headings[x][0] + '</a>';
                // h_aux = '<div class="row"><div class="span12 sm_section"><a href="#title_h1_' + x + '" class="h2_title">' + headings[x][0] + '</a>';
            }
            if (headings[x].length > 1) {
                h_aux = h_aux + '<ul>';
                for (i = 1; i < headings[x].length; i++) {
                    ...
                    if (hasChild) {
                        h_aux = h_aux + '<li><span class="h3_title"><a href="#' + cleanAnchors(headings[x][i], h3_id) + h3_id + '">' + headings[x][i] + ' </a><img src = "https://cdn2.hubspot.net/hubfs/486579/LegalParse/i/arrow_up.svg" class="nav_arrow_h3 expanded"></></span></li>';
                        //h_aux = h_aux + '<li><span class="h3_title"><a href="#title_h2_' + h3_id + '">' + headings[x][i] + ' </a><img src = "https://cdn2.hubspot.net/hubfs/486579/LegalParse/i/arrow_up.svg" class="nav_arrow_h3 expanded"></></span></li>';
                    } else {
                        h_aux = h_aux + '<li class="h3_title"><a href="#' + cleanAnchors(headings[x][i], h3_id) + h3_id + '">' + headings[x][i] + '</a></li>';
                        //h_aux = h_aux + '<li class="h3_title"><a href="#title_h2_' + h3_id + '">' + headings[x][i] + '</a></li>';
                    }
                    if (h4s_parents) {
                        var z = 0;
                        h_aux = h_aux + '<ul>';
                        h4s_parents.forEach(function (parent) {
                            if (headings[x][i] == parent) {
                                for (x2 = 0; x2 < h4s_elements[z].length; x2++) {
                                    h_aux = h_aux + '<li><a href="#' + cleanAnchors(h4s_elements[z][x2], h4_id) + h4_id + '">' + h4s_elements[z][x2] + '</a></li>';
                                    //h_aux = h_aux + '<li><a href="#title_h3_' + h4_id + '">' + h4s_elements[z][x2] + '</a></li>';
                                    h4_id++;
                                }
                            }
                            z++;
    ...
    */
    function replaceIdentifiers() {
        ...
        addID($('h1', content), 0, 'h1');
        addID($('h2', content), 0, 'h2');
        addID($('h3', content), 0, 'h3');
    }

    function addID(aux_hs, x, h) {
        $.each(aux_hs, function (x) {
            $(this).attr('id', cleanAnchors($(this).text() + x));
        });
    ...
    */
    function divideContent() {
        ...
        $('h1', dummy).each(function (item) {
            ...
            h_div += '<div class="content_h2"><img src="https://cdn2.hubspot.net/hubfs/486579/LegalParse/i/arrow_up.svg" class="text_arrow expanded">'; //Add arrow
    ...
    function generateAccordion() {
        ...
        $('.h3_title', '#leftnav').each(function () {
            $(this).nextUntil("h1").not("li").each(function () {
        ...
        $('.h2_title', '#leftnav').each(function () {
            $(this).nextUntil("h1").not("img").each(function () {
                $(this).toggleClass("hide");
            })
            $(this).nextUntil("h1").not("ul").each(function () {
                $(this).on('click', function (event) {
                    if ($(this).hasClass("collapsed")) {
                        $(this).attr("src", "https://cdn2.hubspot.net/hubfs/486579/LegalParse/i/arrow_up.svg");
                        $(this).toggleClass("expanded collapsed");
    ...
    function changeHTMLTag() { //Replaces H1s for H2s to avoid possible issues with many H1s
        ...
        for (var i = 2; i < els.length; i++) {
            console.log('TITLE! --> ' + cleanAnchors(headings[x][0], x));
            els[i].outerHTML = '<h2 id="' + cleanAnchors(headings[x][0], x) + x + '">' + els[i].innerHTML + '</h2>';
            /*if (x != -1) {
                els[i].outerHTML = '<h2 id="' + cleanAnchors(headings[i][0], x) + i + '">' + els[i].innerHTML + '</h2>';
            }*/
            x++;
        }
    }
